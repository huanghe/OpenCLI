...
| `opencli xiaohongshu comment` | Post a top-level comment on a note through the page's own signed request layer (requires `--execute`) |
| `opencli xiaohongshu comment-delete` | Delete one of your own comments on a note (requires `--execute`) |
...
# Post a comment on a note (nothing is sent without --execute)
opencli xiaohongshu comment "https://www.xiaohongshu.com/explore/<id>?xsec_token=..." "写得很好，学到了" --execute

# Delete one of your own comments, by the comment_id the post above printed
opencli xiaohongshu comment-delete "https://www.xiaohongshu.com/explore/<id>?xsec_token=..." 6aa6bdb0000000000b0074cb --execute
...
> `comment` posts a top-level comment on a note and refuses to send anything without `--execute` (without it you get an `ArgumentError` before any navigation or request). Like `follow`, it does not click the UI — the send button is intercepted by overlays here, and a click that quietly does nothing looks exactly like one that posted. There is no Pinia action for posting a comment either (the note store holds only comment *view* state), so the command reaches the note page's own bundled API module through webpack's runtime and calls the wrapper that posts to `/api/sns/web/v1/comment/post`, which is how the signed `X-s`/`X-t`/`X-S-Common` headers get produced. That wrapper is located by matching the endpoint in its source, not by its minified export name, which changes on every xiaohongshu build. The target should be a full signed note URL with `xsec_token`; a bare 24-character hex note ID is accepted but often fails to open. Navigation is paced by the shared risk-control helper, and the command posts **exactly once** — it never retries, because a retried comment is a duplicate comment.
> On success the row is `status: "success"` with `comment_id`, `verified: true`, and `url` pointing at the comment (`<note url>#comment-<id>`). When xiaohongshu accepts the write but returns no comment id, the command **still reports success** (`status: "posted-unverified"`, `verified: false`, `url` = the note URL) and prints `COMMENT_UNVERIFIED` on stderr — the same contract as `follow`'s `FOLLOW_UNVERIFIED`. Treat that as "sent, go check the note", never as a failure to retry.
> `comment-delete` removes one of your own comments and takes the `comment_id` that `comment` printed (the `comments` read command does not expose ids). It goes through the same bundled API module, calling the wrapper for `/api/sns/web/v1/comment/delete` — on the web the "…" affordance only appears while the pointer is over the comment and the menu then opens on *click*, not on hover — reading it as a hover-expanded menu is the natural mistake and wastes a lot of time, and long-pressing your own comment in the mobile app opens the note share sheet rather than a delete option. Its error handling is deliberately the **opposite** of `comment`'s: posting is not idempotent, so an accepted-but-unconfirmed post is reported as success to stop callers double-posting, whereas deleting is idempotent, so an unconfirmed delete **fails loudly** — re-running it is harmless, while a false success would leave the comment up. `--execute` is required for both.
